module Matrix (Matrix,
  ...
  map, map2, indexedMap, filter, mapArrayRows, mapListRows, indexedMapListRows,
  toArrays, toLists
  ) where
{-|
...
# Applying functions
@docs filter, map, map2, indexedMap, toIndexedArray, mapArrayRows, mapListRows, indexedMapListRows
...
-}
mapArrayRows : (Array a -> b) -> Matrix a -> Array b
mapArrayRows func matrix =
  let
    arrays = toArrays matrix
  in
...
-}
mapListRows : (List a -> b) -> Matrix a -> List b
mapListRows func matrix =
  let
    lists = toLists matrix
  in
    List.map func lists

{-| Same as mapListRows, but the function is also applied to the
    index of each row (starting at zero).
-}
indexedMapListRows : (Int -> List a -> b) -> Matrix a -> List b
indexedMapListRows func matrix =
  let
    lists = toLists matrix
  in
    List.indexedMap func lists
